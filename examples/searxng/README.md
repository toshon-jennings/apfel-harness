...
```
model ──asks──▶ apfel ──runs──▶ mcp-websearch.js ──HTTP──▶ SearXNG (localhost:8999)
   ▲                                                              │
...
2. **Run** — localhost-only, host 8999 → container 8080 (8080 is vLLM's):
   ```bash
...
3. **Verify JSON** (must return JSON, not HTML):
   ```bash
   curl "http://127.0.0.1:8999/search?q=test&format=json" | head -c 200
   ```
...
The MCP server reads `SEARXNG_URL` (default `http://127.0.0.1:8999`) and
`SEARX_RESULTS` (default 4). Port 8999 is registered in `PORTMASTER.md`.
